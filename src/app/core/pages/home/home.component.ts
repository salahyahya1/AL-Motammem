...
export class HomeComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  // ✅ Map for Mobile Long Sections
  private sectionMap: Map<string, number> = new Map();

  // =================== DESKTOP SNAP STABILITY ===================
  ...
  private desktopSnapLockUntil = 0;
  private readonly DESKTOP_SNAP_LOCK_MS = 900;
  ...
  // ✅ prevent 2nd auto-snap after programmatic snap
  private mobileSnapLockUntil = 0;
  private readonly MOBILE_SNAP_LOCK_MS = 900;

  // =================== DESKTOP ONLY: AUTO REFRESH ===================
  private resizeObserver?: ResizeObserver;
  private refreshTimeout: any;
  ...
  ngAfterViewInit(): void {
    ...
    // ✅ Mobile path FIRST (بدون pin-ready / ResizeObserver)
    if (this.isMobile) {
      setTimeout(() => this.initMobileSnap(), 750);
      return;
    }

    // ✅ Desktop only: pin-ready + resizeObserver
    window.addEventListener('pin-ready', this.onPinReady);
    this.initResizeObserver();
  ...
  private debouncedRefresh() {
    ...
    this.refreshTimeout = setTimeout(() => {
      if (this.isMobile) return; // ✅ safety
  ...
  private initDesktop(smoother: any) {
    ...
    // ✅ capture scrollbar drag end + direction
  ...
  private buildSnapPositions(smoother: any) {
    ...
    panels.forEach((panel, index) => {
      ...
      // Section4 pinned (desktop) => add end
      if (index === 3) {
        this.snapPositions.push(st.end);
      }

      // Section5 video point
      if (index === 4) {
        this.section5Start = st.start;
        this.section5VideoPoint = (st.end - vh) + 80;
        if (this.section5VideoPoint > this.section5Start + 100) {
  ...
  private doSnap() {
    ...
    // Sec5 relaxed snap (as-is)
    const withinSec5 = (currentScroll > this.section5Start - 100 && currentScroll < this.section5VideoPoint + 200);
    if (withinSec5) {
      const isStart = Math.abs(nearest - this.section5Start) < 2;
      const isVideo = Math.abs(nearest - this.section5VideoPoint) < 2;

      if (isVideo) {
        if (minDistance > 250) return;
      } else if (isStart) {
        if (minDistance > 200) return;
      } else {
        return;
    ...
    if (currentScroll > lastSnap + 200) return;

    if (minDistance > 5) {
      const baseOffset = 50;
      let target = nearest;

      // no offset for video point
      if (Math.abs(nearest - this.section5VideoPoint) < 1) {
        target = nearest;
      } else if (nearest > 0) {
        const idx = this.snapPositions.indexOf(nearest);
        const next = this.snapPositions[idx + 1];
        const maxAllowedOffset = next ? Math.max(0, next - nearest - 20) : baseOffset;
        const safeOffset = Math.min(baseOffset, maxAllowedOffset);
        if (!next || safeOffset >= 8) target = nearest + safeOffset;
      }
  ...
  private initMobileObserver() {
    ...
    this.mobileObserver = ScrollTrigger.observe({
      ...
      onStop: () => {
        if (this.isTouchingMobile) return;
        if (performance.now() < this.mobileSnapLockUntil) return; // ✅ lock guard
        this.doSnapMobile();
      },
      onStopDelay: 0.22,
    });
  ...
  private doSnapMobile() {
    ...
    // ✅ LONG SECTIONS LOGIC (S3/S4/S5/S7)
    const longSections = [
      { id: 'section3-home', nextId: 'section4-home', padding: 0 },
      { id: 'section4-home', nextId: 'section5-home', padding: 0 },
      { id: 'section5-home', nextId: 'section6-home', padding: 80 },
      { id: 'section7-home', nextId: 'section8-home', padding: 0 }
    ];
    ...
    for (const config of longSections) {
      ...
      if (start !== undefined && nextStart !== undefined) {
        if (current >= start - 5 && current < nextStart - 5) {
          ...
          // ✅ free scroll deep inside + gentle snap only when scrolling UP and very close
          if (overlap < 10) {
            if (this.lastDirMobile < 0 && Math.abs(current - start) < 28) {
              this.performSnap(start, current);
            }
            return;
          }
  ...
  private performSnap(target: number, current: number) {
    ...
    // ✅ lock until after tween (prevents double snap)
  ...
  private onResizeMobile = () => {
    ...
    // ✅ MOBILE: rebuild anchors only (NO refresh storm)
