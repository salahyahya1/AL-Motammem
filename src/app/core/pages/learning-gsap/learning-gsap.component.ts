  ...
  this.ngZone.runOutsideAngular(() => {
    requestAnimationFrame(() => {
      document.fonts.ready.then(() => {
        ...
        // 🔹 إعداد البداية للأرقام
        rows.forEach(row => {
          const numEl = row.querySelector('.num') as HTMLElement | null;
          if (numEl) {
            gsap.set(numEl, { opacity: 0 }); // fade فقط
          }
        });
        ...
        rows.forEach((row, i) => {
          ...
          // 🟢 الليبل – كلمات – fade من اليمين للشمال
          if (labelWords) {
            rowTL.to(
              labelWords,
              {
          ...
          // 🔵 الرقم – fade تدريجي + عدّاد في نفس اللحظة
          if (numEl && counterEl) {
            rowTL.to(
              numEl,
              {
