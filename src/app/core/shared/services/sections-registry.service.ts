...
export class SectionsRegistryService {
    private _sections$ = new BehaviorSubject<SectionItem[]>([]);
    // ✅ المهم: observeOn(asyncScheduler) يخلي أي emission بعد دورة الـ CD الحالية
    sections$ = this._sections$.asObservable().pipe(observeOn(asyncScheduler));

    private _enabled$ = new BehaviorSubject<boolean>(false);
    enabled$ = this._enabled$.asObservable().pipe(observeOn(asyncScheduler));
